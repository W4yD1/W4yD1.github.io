...
# Abstract
许多恶意软件通过加壳来绕过检测，因此准确高效的加壳器识别（Packer Identification）方法对恶意软件检测来说十分重要，本文给出```2-SPIFF```——一种2段式的加壳器识别方法，阶段一通过从程序的FCG图中提取特征来辨别程序是否加壳，阶段二通过从FCG图和文件属性中提取的融合特征来识别加壳器。
...
{% note success %}
如果某种加壳器加壳后的程序的FCG与原程序的FCG一致，改检测方法还有效吗？
{% endnote %}
